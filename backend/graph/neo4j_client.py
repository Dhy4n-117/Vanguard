...
class Neo4jClient:
    ...
    def get_subgraph_for_log(self, log_id: str) -> dict:
        """Fetch the subgraph immediately connected to a specific LogEntry id."""
        query = """
        MATCH (le:LogEntry {id: $log_id})-[r]-(m)
        RETURN 
            elementId(le) AS source_id, labels(le)[0] AS source_label, properties(le) AS source_props,
            elementId(m) AS target_id, labels(m)[0] AS target_label, properties(m) AS target_props,
            type(r) AS rel_type, properties(r) AS rel_props,
            startNode(r) = le AS is_outgoing
        """
        nodes_map = {}
        links = []
        
        for record in self.run_cypher(query, {"log_id": log_id}):
            # Source
            s_id = record["source_id"]
            if s_id not in nodes_map:
                s_props = record["source_props"]
                s_name = s_props.get("name") or s_props.get("hostname") or s_props.get("address") or s_props.get("cve_id") or s_props.get("event_type") or str(s_props.get("id", ""))
                nodes_map[s_id] = {"id": s_id, "label": record["source_label"], "name": s_name, "properties": s_props}
            ...
            if t_id not in nodes_map:
                t_props = record["target_props"]
                t_name = t_props.get("name") or t_props.get("hostname") or t_props.get("address") or t_props.get("cve_id") or t_props.get("event_type") or str(t_props.get("id", ""))
                nodes_map[t_id] = {"id": t_id, "label": record["target_label"], "name": t_name, "properties": t_props}
